...
echo "$($PSQL "SELECT name FROM games FULL JOIN teams ON  games.winner_id=teams.team_id OR games.opponent_id=teams.team_id WHERE name LIKE 'Co%' GROUP BY name ORDER BY name")"
